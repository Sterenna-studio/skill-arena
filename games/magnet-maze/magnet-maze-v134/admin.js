(() => {
  ...
  function step(dt) {
    ...
    // NOTE: even when paused, we still update pulse timers + magnet position,
    // so Agent B can reposition and everyone sees the magnet.
    ...
    // If paused, freeze physics/object progression but keep magnet updates alive.
    if (!sim.running) {
      broadcastAcc += dt;
      if (broadcastAcc >= 0.1) { // ~10 Hz while paused
        broadcastAcc = 0;
